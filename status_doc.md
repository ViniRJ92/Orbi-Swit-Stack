...
## Fase 30 (v0.33.0) — Contagem do Analytics pela conversa aberta (corrige sub-registro da conta ativa)

**Pedido do usuário**: a Analytics de "Volume total"/"Instância líder" (`analyticsStore.ts`, baseada só no contador de não lidas do título) estava gerando dados errados, e a instância líder às vezes sumia da lista.

**Causa raiz confirmada antes de implementar**: estrutural do método de badge — o WhatsApp marca uma mensagem como lida quase instantaneamente quando a conversa já está aberta na tela; o título nunca sobe, `AnalyticsStore.observe()` nunca vê um delta, e o total daquela conta fica em 0, exatamente por que `buildSummary()` a filtra da lista (`byAccount.filter(a => a.total > 0)`) e ela pode nunca virar `leader`. Note-se que isto é DIFERENTE do que a Fase 28 (v0.31.0) resolveu: aquela corrigiu a atribuição de DIA (Hoje x Ontem) de mensagens já capturadas pelo badge da lista lateral; esta Fase 30 corrige mensagens que o badge (de conta OU de conversa) nunca chega a capturar de jeito nenhum, porque a conversa está aberta e sendo olhada em tempo real.

**Conflito de escopo levantado e resolvido com o usuário antes de implementar**: ler bolhas de mensagem dentro da conversa aberta é, pela própria regra do projeto ("nada de scraping de conteúdo de mensagens além do já implementado"), uma expansão de escopo — o método anterior (Fase 17/28) nunca entrava numa conversa, só lia a lista lateral. Perguntado explicitamente e autorizado pelo usuário (2026-08-29): ler `data-id` (identificador opaco, nunca texto/remetente/mídia) + qual divisor de data cada bolha está, só da conversa que já estiver aberta (nunca abre/clica em nada) — como **complemento** ao método de badge, não substituição.

**Implementação**:
- `viewManager.ts` — novo `getOpenChatMessages(accountId)`, adicionado sem tocar em `getChatEntries` (Fase 17/28): lê o painel `#main` (só existe quando há conversa aberta — sinal robusto de "tem conversa aberta"), caminha bolhas (`[data-id]`) e prováveis divisores de data (texto "HOJE"/"ONTEM", best-effort) em ordem de documento, mantendo um "bucket" corrente. Filtra mensagens enviadas pelo próprio usuário (`data-id` começando com `true_`) e fora de Hoje/Ontem.
- `analyticsStore.ts` — novo canal `observeOpenChatMessages(accountId, messages)`: deduplicação por identidade (`data-id`), não por delta — cada mensagem só gera 1 evento na vida do app, guardado em `processedMessageIds` (persistido, capado em 3000 IDs/conta). Enquanto uma conta tem `hasOpenChat === true`, ela entra em `openChatAccounts` e o canal de badge (`observe()`) a ignora completamente. `onAccountChatClosed()` devolve a conta ao canal de badge. Os eventos vão para o MESMO array `this.data.events` que já alimenta `buildSummary()` — a correção do "líder sumindo da lista" é consequência direta de corrigir o sinal de entrada, sem tocar na agregação/UI. Nota: esta é uma métrica por CONTA (não por conversa) e vive num arquivo/store diferente (`analytics.json`) do relatório Hoje/Ontem da Fase 28 (`chatActivity.json`) — as duas convivem sem conflito.
- `main.ts` — o mesmo polling de 4s que já lia `getChatEntries` (Fase 17/28) agora também chama `getOpenChatMessages` por conta carregada, sem alterar a chamada já existente.
- Nenhuma mudança em `preload.ts`, `renderer/types.ts`, `useAppStore.ts` ou IPC foi necessária — a UI já consumia `summary.byAccount`/`summary.leader`/`summary.totalVolume`, que passam a vir corretos assim que o novo canal alimenta os mesmos eventos.

**Verificação**: `tsc --noEmit` (main e renderer) sem erros; `npm run build` sem erros, hashes do renderer idênticos aos do instalador oficialmente publicado; smoke test com o app real aberto (log de diagnóstico sem exceções por múltiplos ciclos do poll de 4s).

## Fase 30.1 (v0.33.1) — Reescrita do canal de conversa aberta: de polling para event-driven

**Problema reportado pelo usuário no mesmo dia**: a lógica de varrer a tela do chat a cada 4s (polling) para contar mensagens por `data-id` tinha risco real de duplicar dados ao recarregar o app ou trocar de instância, e dependia de um parsing frágil de texto ("HOJE"/"ONTEM") para decidir a data.

**Esclarecimento de escopo dado ao usuário**: um pedido explícito de "escutar `onMessageReceived`/WebSocket do WhatsApp Web" foi recusado — isso exigiria interceptar o protocolo interno do WhatsApp ou "hookar" o Store JS interno deles (a técnica usada por bibliotecas como whatsapp-web.js), engenharia reversa proibida pela Regra 1 do projeto. Em vez disso, implementada a alternativa que entrega o mesmo resultado técnico sem violar a regra: um `MutationObserver` dentro da própria página, reagindo a inserções reais no DOM.

**Reescrita**:
- `webviewPreload.ts` — novo par `syncChatPanelState()`/`attachMessageObserver()`/`detachMessageObserver()`. Assim que o painel `#main` aparece, TODAS as bolhas já presentes viram baseline (`seenMessageIds`) e nunca são reportadas; só `addedNodes` de mutações subsequentes são inspecionados. Cada mensagem nova dispara `ipcRenderer.send('mw:new-message', { dataId, ts })` no instante exato da inserção — `ts` já é o timestamp real de chegada, sem parsing de "Hoje"/"Ontem". Ao trocar de conversa (painel `#main` desmontado/remontado), o observador é trocado e a baseline resetada. Registra tudo isso assim que a página carrega — não depende do usuário clicar na conta, inclusive a primeira instância que abre com o app.
- `viewManager.ts` — removido `getOpenChatMessages()` (polling). Novos listeners `ipcMain.on('mw:new-message', ...)`/`ipcMain.on('mw:chat-open-state', ...)`, expostos via `setNewMessageListener()`/`setChatOpenStateListener()`.
- `analyticsStore.ts` — `observeOpenChatMessages(accountId, messages[])` (lote, por poll) substituído por `markChatOpen(accountId)` + `recordNewMessage(accountId, dataId, ts)` (1 mensagem por chamada, por evento). `processedMessageIds` continua existindo só como rede de segurança contra reciclagem de nó do DOM na lista virtualizada — não é mais o mecanismo principal de "nunca contar histórico" (isso agora é estrutural: o observador não existe antes da conversa abrir).
- `main.ts` — removida a chamada de polling à conversa aberta do loop de 4s (que continua existindo só para `getChatEntries`, Fase 17/28, inalterado).

**Verificação end-to-end (2026-08-29)**: sem uma sessão WhatsApp logada disponível para teste ao vivo neste ambiente, a cadeia completa foi validada injetando DOM sintético via CDP (Chrome DevTools Protocol) na página real do WhatsApp Web carregada pelo app: (1) painel `#main` fake criado → confirmado eventos `mw:chat-open-state` fluindo; (2) bolha `[data-id]` fake inserida → confirmado exatamente 1 evento gravado em `analytics.json`, mapeado pra conta certa; (3) o MESMO `data-id` reinserido (simulando reciclagem de nó) → confirmado ZERO duplicação; (4) conversa "fechada" (removido `#main`) e uma conversa NOVA aberta já com 2 mensagens "antigas" carregadas → confirmado que nenhuma das duas foi contada; (5) mensagem genuinamente nova nessa conversa nova → confirmado 1 evento novo. Dados sintéticos de teste removidos de `analytics.json` ao final, sem afetar dados reais de outras contas. `tsc --noEmit` e `npm run build` sem erros.

## Fase 30.2 (v0.33.2) — Corrige condição de corrida no carregamento do histórico da conversa

**Problema encontrado em uso real pelo usuário (mesmo dia)**: logo depois de publicada a Fase 30.1, o usuário testou com contas reais conectadas e reportou números muito acima do real — ex.: "Praça Seca 2" mostrou 14 mensagens no Analytics quando só 5 chegaram de verdade naquele dia. Investigação direta no `analytics.json` real (não em ambiente de teste) mostrou a causa exata: 13 das 14 mensagens tinham o MESMO timestamp, no milissegundo — sintoma de um lote só, não de mensagens chegando uma a uma.

**Causa raiz**: o WhatsApp Web não popula o histórico da conversa no mesmo instante em que o container `#main`/painel de mensagens aparece — ele nasce vazio e as mensagens do histórico chegam um instante depois, em um lote assíncrono só. A Fase 30.1 tirava o "snapshot" de baseline no instante em que detectava o painel, cedo demais: o snapshot pegava o painel ainda vazio, e o lote de histórico que chegava logo depois era capturado pelo `MutationObserver` como se fossem mensagens novas.

**Correção**: `attachMessageObserver` (`webviewPreload.ts`) ganhou uma janela de "carregamento" (`MESSAGE_ARM_DELAY_MS = 2000`) — tudo que for inserido no painel nos primeiros 2s depois dele aparecer entra na baseline (nunca é reportado), mesmo que apareça em lote depois do snapshot inicial; só depois dessa janela o observador passa a tratar inserções como mensagens novas de verdade. Mesmo padrão de "estabilização" já usado no `SETTLE_MS` de `analyticsStore.ts` (Fase 15), aplicado à mesma classe de problema (renderização do WhatsApp Web em mais de uma etapa). Limitação assumida e documentada no código: uma mensagem genuinamente nova que chegue dentro dessa janela de 2s não é contada — troca deliberada para nunca mais contar histórico como novo.

**Verificação**: como o ambiente de teste não tem conectividade real com o WhatsApp (nem QR Code funcional), a correção foi validada com um teste sintético de timing controlado (via `performance.now()`/`setTimeout` dentro da própria página, não por espera externa) rodado numa instância com `--user-data-dir` isolado (nunca tocando nos dados reais do usuário): painel vazio criado, 16 mensagens de "histórico" inseridas ~1,2s depois em lote, 1 mensagem genuinamente nova inserida ~5,2s depois — resultado: exatamente 1 evento gravado, as 16 de histórico não entraram nem na deduplicação nem na contagem. **Ressalva importante e comunicada ao usuário**: isso comprova a lógica do código, não substitui validação com uma mensagem real chegando numa conta de verdade — isso só o usuário consegue confirmar, e foi pedido explicitamente que ele faça esse teste após instalar esta versão.

## Fase 30.3 (v0.33.3) — Leva o canal de evento pro relatório Hoje/Ontem por instância (não só Volume total)

**Erro identificado pelo usuário**: as Fases 30/30.1/30.2 corrigiram o canal de evento (MutationObserver, sem polling) só para "Volume total"/"Instância líder" (`analyticsStore.ts`, agregado por CONTA). O card que o usuário realmente estava testando e apontando como quebrado — **"Atividade de hoje"/"Atividade de ontem"** (`chatActivityStore.ts`, formato `Instância - X interações | Y mensagens`, agregado por CONTATO/dia) — continuou usando só o mecanismo antigo (lista lateral, badge de não lidas por conversa, a cada 4s) durante todo esse tempo. Por isso os testes com contas reais mostravam "Atividade de hoje/ontem" sempre em 0, mesmo com mensagens reais chegando: o problema relatado (contagem incorreta ao responder um contato em tempo real) nunca tinha sido tocado — a correção estava sendo aplicada no relatório errado.

**Especificação completa fornecida pelo usuário, usada como base desta correção**:
1. Exibição agregada por instância (nunca nome de contato na tela) — já era assim, sem mudança de UI necessária.
2. 1 pessoa = 1 "interação" por dia, não importa quantas mensagens mande naquele dia; cada mensagem nova dela soma nas "mensagens" do dia. (Ex.: Seu Zé manda 2 de manhã e 3 à tarde → ainda 1 interação, mas 5 mensagens.)
3. Deduplicação por `data-id`: reabrir uma conversa com mensagens já contadas nunca as recontam; só mensagens novas de verdade somam.
4. Mesma regra vale para o bloco "Ontem".
5. A primeira instância que abre com o app já começa a ser observada, sem precisar clique do usuário; nenhuma instância com atividade real deve ficar de fora da lista.

**Implementação**:
- `webviewPreload.ts` — cada evento de "conversa aberta" agora também lê o NOME do cabeçalho da conversa (`extractChatKey`, mesmo seletor `span[title]` já usado na lista lateral) e se é grupo, e reporta via `mw:chat-open-state`. `syncChatPanelState` passou a rodar a CADA tick (não só quando `#main` aparece/some) comparando o nome lido contra o anterior — necessário porque o WhatsApp Web pode manter o mesmo elemento `#main` ao trocar de conversa, só troca o conteúdo; sem essa checagem, trocar de contato não resetava a baseline e o histórico da conversa nova seria contado como mensagem nova (mesmo bug da Fase 30.2, disparado por outro caminho).
- `viewManager.ts` — `mw:new-message`/`mw:chat-open-state` agora carregam `chatKey`/`isGroup`; um mapa interno (`currentChat`) lembra qual conversa está aberta em cada conta entre um evento de mensagem e outro.
- `chatActivityStore.ts` — novo canal de evento: `markChatOpen(accountId, chatKey)` desliga `observe()` (lista lateral) só para AQUELA conversa específica (as outras conversas da mesma conta continuam pela lista lateral normalmente); `recordLiveMessage(accountId, chatKey, dataId, ts)` grava 1 evento por mensagem nova, deduplicado por `data-id` (`processedLiveMessageIds`, mesmo padrão de teto do `analyticsStore.ts`), com o dia (`dayKey`) calculado direto do timestamp real de chegada — não depende mais do rótulo "Hoje"/"Ontem" por texto para este canal. Grupos nunca entram (filtrado em `main.ts` antes de chamar).
- `main.ts` — os listeners de evento agora alimentam os DOIS relatórios: `analyticsStore.recordNewMessage` (sempre) e `chatActivityStore.recordLiveMessage` (só conversas individuais).
- Nenhuma mudança na agregação (`buildDailyReport`/`buildReport`) foi necessária — a lógica de "contagem de contatos distintos = interações, soma = mensagens" já estava correta desde a Fase 28; o problema era só a fonte de dado não cobrir a conversa aberta em tempo real.

**Pendência assumida, não resolvida nesta fase**: possível perda de observação para contas que suspendem/reativam rapidamente (ex.: "Taquara 2" citado pelo usuário como sumindo da lista) — se uma conta for suspensa no meio da janela de estabilização do canal de badge (`SETTLE_MS`), a leitura pendente é descartada, não commitada. Levantado para o usuário confirmar se é esse o caso antes de investir numa correção separada.

**Verificação**: `tsc --noEmit` sem erros. Build gerado e entregue ao usuário para teste com contas reais ANTES de publicar Release — não repetido o erro de publicar sem confirmação prévia desta vez.
